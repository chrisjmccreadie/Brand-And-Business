...
class Javascript extends CI_Controller {
	 ...
	 * @return	string
	 */
	public function load_ckeditor_plugin($loadfile = 'default')
	{
	...
	private function _load($loadfile)
	{
		...
		elseif ($loadfile == 'ui')
		{
			$file = PATH_JQUERY.'ui/jquery.ui.all.js';
		}
...
/*
	...
	private function _generate_mojo_json($page)
	{
		...
		if ($this->auth->is_editor())
		{
			$page['URL']['editor_skin_path'] 	= base_url().SYSDIR.'/mojomotor/views/themes/'.$this->theme.'/editor_skin/';
			$page['URL']['editor_plugin_path'] 	= site_url('javascript/load_ckeditor_plugin');
			...
			{
				$page['toolbar'] = array('Save', 'Maximize', '-',
												$this->config->item('wysiwyg_toolbar'),
												'-', 'Cancel');				
			}
			else
			{
				$page['toolbar'] = array('Save', 'Maximize', '-', 'Cancel');
			}
		}
