	...
	function _check_regions($layout_id, $region_type = 'page', $layout_contents = NULL)
	{
		...
		foreach($layout_dom->find('*[class=mojo_'.$region_type.'_region]') as $region)
		{
			...
			if ($region_type == 'global')
			{
				...
				$region->innertext  = "\n<!-- ".$this->lang->line('global_region_comment')." -->".'{mojo:page:global_region id="'.$region->id.'" emb_layout_id="'.$layout_id.'"}'."\n";
			}
			else
			{
				// Replace content with MojoMotor tag
				$region->innertext  = "\n<!-- ".$this->lang->line('page_region_comment')." -->".'{mojo:page:page_region id="'.$region->id.'" emb_layout_id="'.$layout_id.'"}'."\n";
			}
		}
