...
class Editor extends Mojomotor_Controller {
	...
	function delete_file($file_name = '')
	{
		...
		$upload_path = $upload_settings->row('server_path');
	...
	function upload()
	{
		...
		$config['upload_path'] = $upload_settings->row('server_path');

		if ($upload_settings->row('allowed_types') == 'all')
		{
		...
		if ( ! $this->upload->do_upload($field_name))
		{
			...
			{
				$this->upload->error_msg[0] = '"'.$upload_settings->row('name').'" : '.$this->upload->error_msg[0];
			}
	...
	function browse()
	{
		...
		// This is a workaround for servers that require the use of 'index.php?'.  The
		// Query String becomes incorrect in this case, so we have to do some parsing
		// on it first.
		...
		$qs = $this->input->server('QUERY_STRING', TRUE);

		if (strncmp($qs, '/editor/browse/?', 16) == 0 OR strncmp($qs, '/editor/browse/&', 16) == 0)
		{
			$qs = substr($qs, 16);
		}

		parse_str($qs);
		$vars['CKEditorFuncNum'] = $CKEditorFuncNum;

		$upload_settings = $this->upload_model->get_upload();

		$vars['upload_path'] = $upload_settings->row('server_path');
		$vars['upload_url'] = $upload_settings->row('url');

		if ( ! $vars['files'] = $this->upload_model->get_files($upload_settings->row('id')))
		{
		...
		{
			$vars['message'] = $this->lang->line('no_files_found1').' (<em>'.$upload_settings->row('name').'</em>).</p><p>'.$this->lang->line('no_files_found2');
	...
	function update_page_region()
	{
		...
		// This comment is added to assist admins with new regions. If this region is being
		// updated, then it doesn't need to be there any more. Banish it to the Nether Realm!
		$region_comment	= ($region_type == 'global' ? $this->lang->line('global_region_comment') : $this->lang->line('page_region_comment'));
		$content = str_replace("<!-- {$region_comment} -->", '', $content);
