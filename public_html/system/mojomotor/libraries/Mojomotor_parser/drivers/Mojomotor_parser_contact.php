...
class Mojomotor_parser_contact extends CI_Driver {
	...
	public function send()
	{
		...
		// Grab what they provided to start, then let's analyze this sucka. If
		// it's a full url link, its good. If not, does it already have the
		// page/ segment? If not, we'll add it in.
		...
		if ($recipients_hash ==  md5($this->CI->config->item('encryption_key').$raw_recipients))
		{
			...
			// Email message
			// If a message field is available, it will go at the top of the email, followed by custom fields
			$message = ($this->CI->input->post('message')) ? $this->CI->input->post('message', TRUE)."$n$n------------------$n$n" : '';

			// Message can contain any POST submissions
			unset($_POST['recipients'], $_POST['recipients_hash'], $_POST['from'], $_POST['from_hash'], $_POST['subject'], $_POST['message'], $_POST['return']);
